...
def get_text2(phrase_time_limit=10):
    ...
    while True:
        with sr.Microphone() as source:
            ...
            try:
                ...
                key = r.recognize_google(audio, language="en")
                if key == 'hello':
                    print("\nTell me what you need")
                    try:
                        audio_comando = r.listen(source, timeout=5, phrase_time_limit=phrase_time_limit)

                        text = r.recognize_google(audio_comando, language="en")
                        if text:
                            print(f"Você disse: {text}")
                            break
